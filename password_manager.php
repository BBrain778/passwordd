...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add_password'])) {
        ...
        // Verify if the password belongs to the logged-in user before deletion
        $sql = "SELECT id FROM passwordmanage WHERE id = ? AND username = ?";
        $params = array($id, $username);
        $stmt = sqlsrv_query($conn, $sql, $params);

        if ($stmt === false || sqlsrv_has_rows($stmt) === false) {
            die("Unauthorized attempt to delete a password.");
        }

        // Delete the password from the database
        $sql = "DELETE FROM passwordmanage WHERE id = ? AND username = ?";
        $stmt = sqlsrv_query($conn, $params);
